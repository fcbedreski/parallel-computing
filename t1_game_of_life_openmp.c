//**** Code reference: https://rosettacode.org/wiki/Conway%27s_Game_of_Life/C <-------
...
void evolve(const char *field, char *t, int size)
{
   ...
   /* - Assignment comments -
   Perfectly nested loop case
   Collapse directive: treats the nested for as a unique loop of 'sizeXsize' size 
   and parallelizes the structure
   */
  #pragma omp parallel for collapse(2) //Number of loops to be parallelized, including the outer loop
   for(i=0; i < size; i++)
   {
      for(j=0; j < size; j++)
      {
...
/* Author comment: some additional header needed to use the function evolve provided
...
/* Author comment: set the cell i,j as alive */
...
int main(int argc, char **argv)
{
    ...
    /* Author comment: fast and dirty selection option */
    if ( argc > 1 )
    {
      op = *argv[1];
    } else {
    ...
    switch ( op )
    {
      ...
      case 'g':
        ...
        /* Author comment: prepare the glider */
            SCELL(0, 1);
            SCELL(1, 2);
            SCELL(2, 0); 
            SCELL(2, 1); 
            SCELL(2, 2);
        /* Author comment: evolve */
        fa = field;
        fb = temp_field;
